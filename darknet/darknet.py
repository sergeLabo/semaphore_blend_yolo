...
To use, either run performDetect() after import,
or modify the end of this file.

See the docstring of performDetect() for parameters.

Directly viewing or returning bounding-boxed images requires scikit-image
to be installed (`pip install scikit-image`)


Original *nix 2.7:
https://github.com/pjreddie/darknet/blob/
0f110834f4e18b30d5f101bf8f1724c34b7b83db/python/darknet.py

Windows Python 2.7 version:
https://github.com/AlexeyAB/darknet/blob/
fc496d52bf22a0bb257300d3c79be9cd80e722cb/build/darknet/x64/darknet.py
...
def detect(net, meta, image, thresh=0.5, hier_thresh=0.5,
    ...
    if debug:
        print("Loaded image")
    ret, tag, confiance = detect_image(net, meta, im, thresh,
...
def detect_image(net, meta, im, thresh=0.5, hier_thresh=0.5, nms=0.45,
                 debug=False):
    tag = None
    confiance = None
    num = c_int(0)
        ...
        for i in range(meta.classes):
            ...
            if dets[j].prob[i] > 0:
                ...
                if altNames is None:
                    ...
                    nameTag = altNames[i]
                tag = nameTag.decode("utf-8")
                confiance = round(dets[j].prob[i], 2)
                if debug:
                    print("Got", tag , confiance)
...
def performDetect(
    ...
    # This lets us instance Darknet once, then just call performDetect()
    # again without instancing again
    ...
    if not os.path.exists(metaPath):
        raise ValueError(
            "Invalid data file path `" + os.path.abspath(metaPath) + "`"
        )
    if netMain is None:
    ...
    if altNames is None:
        # In Python 3, the metafile default access craps out on Windows
        # (but not Linux)
    ...
    if not os.path.exists(imagePath):
        raise ValueError(
            "Invalid image path `" + os.path.abspath(imagePath) + "`"
        )
    # Do the detection
